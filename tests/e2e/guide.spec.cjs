...
for (const width of [375,768,1280]) {
  test(`guide at ${width}px has no page overflow and supports keyboard navigation`, async ({page}) => {
    ...
    for (const overflow of await page.locator('.guide-content table').evaluateAll(tables=>tables.map(table=>table.getBoundingClientRect().right-table.parentElement.getBoundingClientRect().right))) expect(overflow).toBeLessThanOrEqual(1);
